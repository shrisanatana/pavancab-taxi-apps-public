...
import com.pavancab.niranjan.BuildConfig
...
object UpdateManager {
    ...
    private const val PREFS = "pavancab_update_prefs"
    ...
    suspend fun check(context: Context): UpdateInfo? {
        ...
        return try {
            ...
            val info = UpdateInfo(
                ...
                forceUpdate = force,
                message = runCatching { cfg.get("message").asString }.getOrElse { "A new version of PavanCab is available." },
                playStoreUrl = runCatching { cfg.get("playStoreUrl").asString }.getOrElse {
                    "https://play.google.com/store/apps/details?id=$pkg"
                }
            )
            // Non-forced updates already dismissed by the user are not nagged again.
