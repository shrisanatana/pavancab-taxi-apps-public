<?php
/**
 * PAVANCAB GOA TAXI - Needs Driver Desk
 * Path: app/dashboard/pending.php
 */

$pageTitle = '🔴 Needs Driver Desk • Immediate Dispatch';
$activeTab = 'PENDING';
require_once __DIR__ . '/_layout_header.php';
?>

<!-- DYNAMIC EMPTY STATE -->
<div id="bookings-empty-state" class="<?php echo $statPending === 0 ? '' : 'hidden'; ?> uber-card p-10 sm:p-12 rounded-3xl border border-dashed border-slate-800 text-center space-y-4 my-4 animate-fadeIn">
  ...
  <div class="space-y-1.5">
    <h3 class="text-base sm:text-lg font-black text-white font-outfit uppercase tracking-wide" id="bookings-empty-title">All Drivers Dispatched • Zero Rides Pending</h3>
    <p class="text-xs text-slate-400 max-w-sm mx-auto leading-relaxed" id="bookings-empty-text">
      Great news! There are currently 0 rides waiting for driver assignment. All current bookings have drivers assigned or are already completed.
    </p>
  </div>
  <div>
    <a href="./index.php" class="bg-slate-900 hover:bg-slate-800 border border-slate-700 text-slate-300 text-xs font-bold px-4 py-2 rounded-xl transition inline-flex items-center gap-1.5 shadow">
      <i data-lucide="layers" class="w-3.5 h-3.5 text-amber-400"></i> View All Rides
    </a>
  </div>
</div>
...
<div class="space-y-4" id="bookings-feed-container">
  ...
  ?>
    <div class="booking-card uber-card p-5 sm:p-6 rounded-3xl border border-red-500/40 bg-red-950/10 space-y-4 shadow-xl transition relative overflow-hidden <?php echo !$isPending ? 'hidden' : ''; ?>" 
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-3.5">
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="text-xs font-black uppercase px-3 py-1.5 rounded-xl border bg-red-500/20 text-red-300 border-red-500/40 animate-pulse">
            NEEDS DRIVER
          </span>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-800/80">
        <div class="flex flex-wrap items-center gap-2">
          <button onclick="openAssignModal(<?php echo $b['id']; ?>)" class="gradient-btn-gold text-slate-950 font-black text-xs px-4 py-2.5 rounded-xl flex items-center gap-1.5 shadow-lg">
            <i data-lucide="user-plus" class="w-4 h-4"></i> 🚀 Dispatch Driver via WhatsApp
          </button>
        ...
        <div class="flex items-center gap-2">
          ...
          <select onchange="updateBookingStatus(<?php echo $b['id']; ?>, this.value)" class="bg-slate-900 border border-slate-700 text-slate-200 text-xs font-bold px-3 py-2 rounded-xl focus:outline-none focus:border-amber-400">
            ...
            <option value="PENDING" selected>PENDING (Needs Driver)</option>
