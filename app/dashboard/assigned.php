<?php
/**
 * PAVANCAB GOA TAXI - Assigned Cabs Desk
 * Path: app/dashboard/assigned.php
 */

$pageTitle = '🔵 Assigned Cabs Desk • Dispatched Drivers';
$activeTab = 'CONFIRMED';
require_once __DIR__ . '/_layout_header.php';
?>

<!-- DYNAMIC EMPTY STATE -->
<div id="bookings-empty-state" class="<?php echo $statAssigned === 0 ? '' : 'hidden'; ?> uber-card p-10 sm:p-12 rounded-3xl border border-dashed border-slate-800 text-center space-y-4 my-4 animate-fadeIn">
  <div class="w-14 h-14 rounded-2xl bg-indigo-500/10 border border-indigo-500/30 flex items-center justify-center mx-auto text-indigo-400 text-2xl">
    🚕
  </div>
  <div class="space-y-1.5">
    <h3 class="text-base sm:text-lg font-black text-white font-outfit uppercase tracking-wide" id="bookings-empty-title">No Assigned Rides Waiting to Start</h3>
    <p class="text-xs text-slate-400 max-w-sm mx-auto leading-relaxed" id="bookings-empty-text">
      No upcoming rides currently in confirmed state. Check Needs Driver to assign new incoming trips.
    </p>
  </div>
  <div>
    <a href="./pending.php" class="gradient-btn-gold text-slate-950 font-black text-xs px-4 py-2 rounded-xl uppercase transition inline-flex items-center gap-1.5 shadow">
      <i data-lucide="alert-circle" class="w-3.5 h-3.5"></i> Check Needs Driver
    </a>
  </div>
</div>
...
<div class="space-y-4" id="bookings-feed-container">
  ...
  ?>
    <div class="booking-card uber-card p-5 sm:p-6 rounded-3xl border border-indigo-500/40 bg-indigo-950/10 space-y-4 shadow-xl transition relative overflow-hidden <?php echo !$isAssigned ? 'hidden' : ''; ?>" 
      ...
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800/80 pb-3.5">
        ...
        <div class="flex items-center gap-3">
          ...
          <span class="text-xs font-black uppercase px-3 py-1.5 rounded-xl border bg-indigo-500/20 text-indigo-300 border-indigo-500/40">
            DRIVER ASSIGNED
          </span>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-slate-800/80">
        <div class="flex flex-wrap items-center gap-2">
          <button onclick="updateBookingStatus(<?php echo $b['id']; ?>, 'IN_TRANSIT')" class="bg-yellow-500 hover:bg-yellow-400 text-slate-950 font-black text-xs px-4 py-2 rounded-xl flex items-center gap-1.5 shadow-md transition">
          ...
          <button onclick="openAssignModal(<?php echo $b['id']; ?>)" class="bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs px-3 py-2 rounded-xl flex items-center gap-1">
            <i data-lucide="user-check" class="w-3.5 h-3.5 text-amber-400"></i> Re-assign Driver
          </button>
        ...
        <div class="flex items-center gap-2">
          ...
          <select onchange="updateBookingStatus(<?php echo $b['id']; ?>, this.value)" class="bg-slate-900 border border-slate-700 text-slate-200 text-xs font-bold px-3 py-2 rounded-xl focus:outline-none focus:border-amber-400">
            ...
            <option value="CONFIRMED" selected>CONFIRMED (Assigned)</option>
            <option value="IN_TRANSIT">IN_TRANSIT (On Trip)</option>
