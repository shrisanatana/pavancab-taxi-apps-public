...
function handleRidesModule($method, $path, $b) {
    ...
    // Helper: Verify caller owns booking
    function verifyBookingOwnership($bk) {
        ...
        $callerEmail = $_SESSION['user']['email'] ?? '';
        ...
        if ($callerEmail && $callerEmail === ($bk['user_email'] ?? '')) return true;
    ...
    if ($method === 'POST' && ($path === '/user/complete-ride' || $path === '/complete-ride')) {
        ...
        dbExec("UPDATE app_bookings SET status = 'COMPLETED', updated_at = NOW() WHERE id = ?", 'i', [$booking_id]);
        if ($bk['driver_id']) dbExec("UPDATE app_drivers SET status = 'available' WHERE id = ?", 'i', [$bk['driver_id']]);
        ...
        // FCM to admins
        try {
            $atokens = getAdminFCMTokens();
            if (!empty($atokens)) sendFCMPush($atokens, "Ride Completed (#$ref)", "Ride #$ref completed. Fare: Rs.$fare");
        } catch (\Throwable $e) {}
