...
if (strpos($action, 'driver_') === 0) {
    ...
    if ($da === 'respond' && $method === 'POST') {
        ...
        if ($decision === 'ACCEPT') {
            ...
            if (empty($sub)) {
                ...
                if ($pendingCount > 0) {
                    $cfgSub = dbRows("SELECT setting_value FROM app_settings WHERE setting_key = 'driver_subscription_amount' LIMIT 1");
                    $cfgComm = dbRows("SELECT setting_value FROM app_settings WHERE setting_key = 'driver_commission_per_ride' LIMIT 1");
                    $subAmt = intval($cfgSub[0]['setting_value'] ?? 1000);
                    $commAmt = intval($cfgComm[0]['setting_value'] ?? 200);
                    jsonResponse(['success' => false, 'error' => 'payment_required', 'message' => "You have $pendingCount unpaid ride commission(s). Pay Ã¢â€šÂ¹" . ($pendingCount * $commAmt) . " or subscribe for Ã¢â€šÂ¹$subAmt/month to accept rides.", 'pending_count' => $pendingCount], 402);
                }
            }
    ...
    if ($da === 'trip-status' && $method === 'POST') {
            ...
            if ($booking['status'] === 'ASSIGNED' || $booking['status'] === 'ACCEPTED') {
                ...
                if (empty($sub)) {
                    ...
                    if ($pendingCount > 0) {
                        ...
                        $subAmt = intval($cfgSub[0]['setting_value'] ?? 1000);
                        $commAmt = intval($cfgComm[0]['setting_value'] ?? 200);
                        jsonResponse(['success' => false, 'error' => 'payment_required', 'message' => "You have $pendingCount unpaid commission(s). Pay Ã¢â€šÂ¹" . ($pendingCount * $commAmt) . " or subscribe for Ã¢â€šÂ¹$subAmt/month to accept rides.", 'pending_count' => $pendingCount], 402);
                    }
                }
            }
        ...
        if ($mapStatus === 'COMPLETED') {
            ...
            if (empty($sub)) {
                ...
                $subCfg = dbRows("SELECT setting_value FROM app_settings WHERE setting_key = 'driver_subscription_amount' LIMIT 1");
                $subAmt = intval($subCfg[0]['setting_value'] ?? 1000);
    ...
    if ($da === 'verify-payment' && $method === 'POST') {
        ...
        if (!$razorpayOrderId || !$razorpayPaymentId) {
            jsonResponse(['error' => 'Missing payment details (order_id and payment_id required)'], 400);
        }

        // Server-side verification via Razorpay API (no client signature needed)
        $keySecret = razorpayKeys();
        $keyId = $keySecret[0];
        $secret = $keySecret[1] ?? '';
        ...
        if ($payHttp !== 200 || empty($payData['id'])) {
            jsonResponse(['success' => false, 'error' => 'Payment verification failed - could not fetch payment details from Razorpay'], 400);
        }

        // Verify the order_id matches
        $payOrderId = $payData['order_id'] ?? '';
        if ($payOrderId !== $razorpayOrderId) {
            jsonResponse(['success' => false, 'error' => 'Payment verification failed - order_id mismatch'], 400);
        }

        // Verify status is captured
        $payStatus = $payData['status'] ?? '';
        if ($payStatus !== 'captured' && $payStatus !== 'authorized') {
            jsonResponse(['success' => false, 'error' => 'Payment not completed. Status: ' . $payStatus], 400);
        }
